...
import com.sky.beautiful.Adapter.CollectionRecyclerViewAdapter1;
 ...
 * @USER : vvguoliang
 * @File : CollectionFragment_2.java
...
public class CollectionFragment_2 extends Fragment implements BGARefreshLayout.BGARefreshLayoutDelegate, BGAOnItemChildClickListener, View.OnClickListener {
    ...
    private CollectionRecyclerViewAdapter1 collectionRecyclerViewAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = LayoutInflater.from(mActivity).inflate(R.layout.far_collection_fragment, container, false);
        findViewById();
        getData();
        return view;
    }
    ...
    private void getData() {
        ...
        collectionRecyclerViewAdapter = new CollectionRecyclerViewAdapter1(rv_recyclerview_data);
    ...
    public void onItemChildClick(ViewGroup parent, View childView1, View childView2, int position) {
        if (bool_image_image) {
            ...
            intent.putExtra("AtlasName", myCollects1.get(position).getAtlasName());
    ...
    private void getImagId(int position, View childView1, View childView2) {
        ...
        for (int i = 0; integerList.size() > i; i++) {
            if (myCollects1.get(position).getImgID().equals(integerList.get(i).get("imgID"))) {
                UtilsInterface.getInstance().UtilBlur(mActivity, "", myCollects1.get(position).getFirstImage(), (ImageView) childView1);
            }
        }
    }
    ...
    private void getMyCollect() {
        HttpRequest.getInstance().setPublic(mActivity,
                AggregateMap.getInstance().setMyCollect(SharedPreferencesUtils.get(mActivity, "userNo", "0").toString(), mNewPageNumber, 2),
    ...
    private void getMyCollet(List<MyCollect> myCollects) {
        if (myCollects != null && myCollects.size() > 0) {
            for (int i = 0; myCollects.size() > i; i++) {
                if (myCollects.get(i).getCollectType() == 2) {
                    MyCollect myCollect = new MyCollect(
                            ...
                            myCollects.get(i).getCollectType(),
                            myCollects.get(i).getAtlasName(),
                            myCollects.get(i).getFirstImage());
